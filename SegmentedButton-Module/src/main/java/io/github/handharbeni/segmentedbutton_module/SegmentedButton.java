package io.github.handharbeni.segmentedbutton_module;
...
import io.github.handharbeni.rippledrawable_module.DrawableHotspotTouch;
...
public class SegmentedButton extends View {
	...
	private io.github.handharbeni.rippledrawable_module.RippleDrawable rippleDrawable;
	...
	private void getAttributes(Context context, @Nullable AttributeSet attrs) {
		...
		if (ta.hasValue(R.styleable.SegmentedButton_selectedBackground))
			selectedBackgroundDrawable =
					ta.getDrawable(R.styleable.SegmentedButton_selectedBackground);
		...
		ellipsize =
				resolveEllipsizeType(ta.getInt(R.styleable.SegmentedButton_android_ellipsize, 0));
		...
		final int
				selectedTextStyle =
				ta.getInt(R.styleable.SegmentedButton_selectedTextStyle, textStyle);
		...
		if (hasFontFamily) {
			...
			if (VERSION.SDK_INT >= VERSION_CODES.O) {
				textTypeface =
						Typeface.create(ta.getFont(R.styleable.SegmentedButton_android_fontFamily), textStyle);
				selectedTextTypeface =
						Typeface.create(ta.getFont(R.styleable.SegmentedButton_android_fontFamily), selectedTextStyle);
			} else {
				final int
						fontFamily =
						ta.getResourceId(R.styleable.SegmentedButton_android_fontFamily, 0);

				if (fontFamily > 0) {
					textTypeface =
							Typeface.create(ResourcesCompat.getFont(context, fontFamily), textStyle);
					selectedTextTypeface =
							Typeface.create(ResourcesCompat.getFont(context, fontFamily), selectedTextStyle);
				} else {
					...
					textTypeface = Typeface.create(
							ta.getString(R.styleable.SegmentedButton_android_fontFamily),
							textStyle
					);
					selectedTextTypeface = Typeface.create(
							ta.getString(R.styleable.SegmentedButton_android_fontFamily),
							selectedTextStyle
					);
				}
	...
	private void initText() {
		...
		if (VERSION.SDK_INT >= VERSION_CODES.M) {
			...
			textStaticLayout =
					new StaticLayout(text, textPaint, textMaxWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0,
					                 false
					);
		}
	}
	...
	private void initDrawable() {
		...
		if (hasSelectedDrawableTint)
			selectedDrawableColorFilter =
					new PorterDuffColorFilter(selectedDrawableTint, PorterDuff.Mode.SRC_IN);
	}
	...
	private void measureTextWidth(int width, int drawableWidth) {
		...
		int
				textWidth =
				Math.min(width - getPaddingLeft() - getPaddingRight() - newDrawableWidth, textMaxWidth);
		...
		if (VERSION.SDK_INT >= VERSION_CODES.M) {
			...
			textStaticLayout =
					new StaticLayout(text, textPaint, textWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0, false);
		}
	}
	...
	private void updateSize() {
		...
		if (Gravity.isHorizontal(drawableGravity)) {
			...
			final float
					startPosition =
					(width - textWidth - drawableWidth - drawablePadding) / 2.0f;
			...
			final float
					startPosition =
					(height - textHeight - drawableHeight - drawablePadding) / 2.0f;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (drawable != null) {
			...
			drawable.setColorFilter(
					hasSelectedDrawableTint ? selectedDrawableColorFilter : drawableColorFilter);
			drawable.draw(canvas);
		}
	...
	void setupBackgroundClipPath() {
		...
		if (isRounded() || (isLeftButton() && isRightButton())) {
			...
			backgroundClipPath.addRoundRect(rectF,
			                                new float[]{br, br, br, br, br, br, br,
					                                br}, Direction.CW
			);
		} else if (isLeftButton()) {
			// Add radius on left side only
			backgroundClipPath = new Path();
			backgroundClipPath.addRoundRect(rectF, new float[]{br, br, 0, 0, 0, 0, br,
					br}, Direction.CW);
		} else if (isRightButton()) {
			// Add radius on right side only
			backgroundClipPath = new Path();
			backgroundClipPath.addRoundRect(rectF, new float[]{0, 0, br, br, br, br, 0,
					0}, Direction.CW);
		} else {
			backgroundClipPath = null;
		}
	...
	void setupBackgroundBitmaps() {
		...
		if ((backgroundClipPath != null || selectedButtonRadius > 0) && selectedBackgroundDrawable != null
			...
			final BitmapShader
					selectedBackgroundBitmapShader =
					new BitmapShader(bitmap, TileMode.CLAMP,
					                 TileMode.CLAMP
					);
	...
	void setSelectedButtonBorder(int width, @ColorInt int color, int dashWidth, int dashGap) {
		if (width > 0) {
			...
			if (dashWidth > 0.0f) {
				selectedButtonBorderPaint.setPathEffect(new DashPathEffect(new float[]{dashWidth,
						dashGap}, 0));
			}
		} else {
	...
	public void setRipple(@ColorInt int color) {
		...
		if (VERSION.SDK_INT >= VERSION_CODES.LOLLIPOP) {
			rippleDrawableLollipop =
					new RippleDrawable(ColorStateList.valueOf(rippleColor), null, null);
			...
			rippleDrawable =
					new io.github.handharbeni.rippledrawable_module.RippleDrawable(ColorStateList.valueOf(rippleColor), null, null);
			// setCallback on Drawable allows animations to be scheduled and the drawable to invalidate the view on
			// animation
